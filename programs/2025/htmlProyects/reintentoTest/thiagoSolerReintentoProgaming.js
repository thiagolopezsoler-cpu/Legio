...
const travelsToAdd = [
    { destino: "Buenos Aires, Argentina", duracion: "5 dias", precio: "$800" },
    { destino: "Roma, Italia", duracion: "5 dias", precio: "$800" }
];
...
travelsToAdd.forEach(({ destino, duracion, precio }) => agregar_viaje(destino, duracion, precio));
...
let allTheTravels = document.getElementsByClassName("viaje");
// console.log(allTheTravels);

for (let x = 0; x < allTheTravels.length; x++) {
    allTheTravels[x].addEventListener("click", () => {
        // let cart = document.getElementById("carrito");
        // let cartValue = parseInt(cart.getElementsByTagName("span")[0].textContent.replace("$", ""));
        let cartValue = parseInt(document.getElementById("carrito").getElementsByTagName("span")[0].textContent.replace("$", ""));
        let value = parseInt(allTheTravels[x].getElementsByTagName("p")[1].textContent.replace("$", ""));
        document.getElementById("carrito").getElementsByTagName("span")[0].textContent = `$${value + cartValue}`
        // console.log(cartValue);
        // cartValue += value;
        // document.getElementById("carrito").getElementsByTagName("span")[0].textContent.replace("$", "") += allTheTravels[x].getElementsByTagName("p")[0].textContent.replace("$", "");
    })
};