import flet as ft
# Importamos nuestras propias herramientas modulares
from modelos import Estacion, Tren, Linea
from database import inicializar_db, validar_login

# Inicializamos la base de datos al arrancar el programa
inicializar_db()

# Tu base de datos simulada en memoria (se mantiene igual)
lineas_db = {}
trenes_db = {}
...
def main(page: ft.Page):
    ...
    def cargar_sistema_metro():
        page.clean() # Borra la pantalla de login por completo
        ...
        borde_gris = ft.Border(
            top=ft.BorderSide(1, "grey800"), bottom=ft.BorderSide(1, "grey800"),
            left=ft.BorderSide(1, "grey800"), right=ft.BorderSide(1, "grey800")
        )
        borde_rojo = ft.Border(
            top=ft.BorderSide(1, "red"), bottom=ft.BorderSide(1, "red"),
            left=ft.BorderSide(1, "red"), right=ft.BorderSide(1, "red")
        )
    ...
    input_user = ft.TextField(label="Usuario", width=280)
    input_pass = ft.TextField(label="Contraseña", password=True, can_reveal_password=True, width=280)
    txt_error = ft.Text("", color="red", weight="bold")

    def intentar_login(e):
        usuario = input_user.value.strip()
        contra = input_pass.value.strip()
        
        # Consultamos a la base de datos real en database.py
        if validar_login(usuario, contra):
            cargar_sistema_metro() # Si es correcto, pasamos al programa
        else:
    ...
    caja_login = ft.Container(
        content=ft.Column([
            ft.Text("Control de Accesos", size=22, weight="bold", color="red"),
            ft.Divider(),
            input_user,
            input_pass,
            txt_error,
            ft.ElevatedButton("Ingresar", bgcolor="red", color="white", on_click=intentar_login, width=280)
        ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
        padding=25, bgcolor="grey900", border_radius=10, width=320
    )
