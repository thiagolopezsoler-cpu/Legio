...
function addProduct(name, category, cost) {
    ...
    <button class="btn" onclick="addPrice(${cost})">Add to the cart</button>`;
...
function addPrice(price) {
    ...
    currentTotal = currentTotal< 5000? price + currentTotal : price * 0.8+ currentTotal;
...
/*
teory<.
1_ No, HTML no es un lenguaje de programacion sino que es un lenguaje marcado(esto quiero decir que marca o etiqueta el contenido
...
2_Meta es una etiqueta que se usa para definir metadatos, es decir, informacion sobre la pagina web como el charset(el tipo de 
caracteres que se van a usar), el viewport(como se comporta la pagina en distintos dispositivos)).

3_ La principal diferencia es que el head hay uno solo por archivo HTML y en el header puede haber varios.
